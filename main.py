...
class Encryption:
    ...
    def roundEndKeyAddition(mixValue, subKey):
        roundEndAddition = []
        for i in range(4):
            roundEndAddition.append([("%0.2X" % (int(mixValue[i][0], 16) ^ int(subKey[i][2:4], 16))),
                                     ("%0.2X" % (int(mixValue[i][1], 16) ^ int(subKey[i][4:6], 16))) \
                                        , ("%0.2X" % (int(mixValue[i][2], 16) ^ int(subKey[i][6:8], 16))),
                                     ("%0.2X" % (int(mixValue[i][3], 16) ^ int(subKey[i][8:], 16)))])
...
class Decryption:
    ...
    def firstKeyDecrypter(subText, subKey):
        return [("%0.2X" % (int(subText[0], 16) ^ int(subKey[2:4], 16))),
                ("%0.2X" % (int(subText[1], 16) ^ int(subKey[4:6], 16))) \
            , ("%0.2X" % (int(subText[2], 16) ^ int(subKey[6:8], 16))),
                ("%0.2X" % (int(subText[3], 16) ^ int(subKey[8:], 16)))]
...
def encryption(AESchoice, fileName, folderPath):
    if AESchoice == None or fileName == "" or folderPath == "":
        ...
        if "/" in folderPath:
            ...
            folderPath = folderPath + "\\"
        progress, name, key = Encryption.run(fileName, AESchoice, folderPath)#прогрес-чи файл розшифровано,name-назва шифрованого файлу,кей-ключ
...
def decryption(AESchoice, fileName, folderPath, formatValue, key):
    if AESchoice == None or fileName == "" or key == "" or folderPath == "":
        ...
        if (AESchoice == '128' and len(key) == 34) or (AESchoice == '192' and len(key) == 50) or (
                AESchoice == '256' and len(key) == 66):
...
encryptButton = Button(encryptIpFrame, text="Start Encryption",
                       command=lambda: encryption(var.get(), window.sourceFile, window.sourceFolder))

decryptButton = Button(decryptIpFrame, text="Start Decryption",
                       command=lambda: decryption(decryptVar.get(), window.sourceFile, window.sourceFolder,
                                                  decryptEnterFormat.get(), decryptEnterKey.get()))
